let biodata = {
    nama: "Ananda Fadhilah Perkasa Putra",
    email: "perkasa.putra1404@gmail.com",
  ...
  //jawaban nomor 1
  function nama(biodata) {
    return `
          Nama Lengkap : ${biodata.nama}\n
          Pernah menempuh pendidikan di ${biodata.riwayatPendidikan.length} tempat
      `
  }
  
  console.log(nama(biodata))
  ...
  function printHobbies(listHobi) {
    ...
    for (let i = 0; i < listHobi.length; i++) {
      result += `Hobi ke ${i + 1} adalah ${listHobi[i]}\n`
    }
    return result
  }
  const listHobi = biodata.hobi
  
  console.log(printHobbies(listHobi))
  ...
  const gradeCheckMessage = checkPassingGrade(grade)
